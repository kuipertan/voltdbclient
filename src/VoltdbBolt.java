...
public class VoltdbBolt implements IRichBolt {
	...
	public void InitConfig(String svs, String user, String pass, String map_context){
		this.servers = svs;
		this.user = user;
		this.pass = pass;
	...
	public void execute(Tuple arg0) {
		...
		*/
			...
			/*if (rs.LoadFromJson(kfk2sql, inputJSONObject));
				if (rs.WriteToVoltdb(client) && inputJSONObject.has("rec_item_ids")){
					try {
						if (inputJSONObject.get("rec_item_ids") instanceof JSONArray){
							JSONArray recs = inputJSONObject.getJSONArray("rec_item_ids");
							for(int i=0; i < recs.length(); ++i)
								rs.WriteResultItem(client, recs.getString(i));
						}
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
