...
public class VoltdbTopology {
	...
	void initConfig(){
		FileInputStream in;
		try {
			in = new FileInputStream("/opt/sys/topology/vdbjars/config.properties");
			prop.load(in);
			in.close();
		} catch (Exception e) {
	...
	public static void main(String[] args) throws Exception {
        ...
        String forceFromStart		= tpl.prop.getProperty("FromBeginning");
        String defaultIntoIidCids	= tpl.prop.getProperty("default_iid_table_cids");
        String cfg_zks	= tpl.prop.getProperty("cfg_zks");
        ...
        if (forceFromStart != null && forceFromStart.compareToIgnoreCase("yes") == 0)
        ...
        builder.setSpout("signal_spout", new ZkCfgSpout(cfg_zks), 1);
        VoltdbBolt bolt = new VoltdbBolt();
        
        File file = new File("/opt/sys/topology/vdbjars/map.json");  
        Long filelength = file.length();  
        byte[] filecontent = new byte[filelength.intValue()];   
        FileInputStream in = new FileInputStream(file);  
        in.read(filecontent);  
        in.close();   
        bolt.InitConfig(servers, usr, pwd, new String(filecontent), defaultIntoIidCids);
        
        builder.setBolt("vdb_bolt-"+topic, bolt, Integer.parseInt(bolt_vdb_parl)).shuffleGrouping(spout_id).allGrouping("signal_spout");
	...
	public static boolean doTest(){
		...
		CommonResult rs = new CommonResult(new VoltdbBolt());
		Event event = new Event(new VoltdbBolt());//
		Request rq = new Request(new VoltdbBolt());
		Item item = new Item();
		News news = new News();
